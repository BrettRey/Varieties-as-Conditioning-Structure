...
What distinguishes register from dialect from discourse community? This paper argues that they differ not in what they are but in what participant expectation is conditioned on: SITUATION, ASCRIPTION, or IDENTIFICATION. These are not just descriptive dimensions. Recurrent situations, social classifications, and norm orientations keep rebuilding the same expectation profiles. The key move is separating ascription (what you are treated as) from identification (whose norms you orient to). Drawing on Wiese's (2023) account of communicative situations and O'Connor's (2019) game-theoretic work on how categories stabilize, I argue that varieties hang together because the same kinds of situations recur, the same kinds of speakers keep getting socially classified in familiar ways, and the same norm centres keep organizing orientation, while social coordination makes established partitions sticky. In homeostatic property cluster terms, a variety is real enough to support prediction: knowing the conditioning state lets you anticipate likely forms, and hearing the forms lets you infer the conditioning state. A worked example using Kiesling's (1998) fraternity data on the sociolinguistic variable traditionally written as (ING) shows that this framework captures structure that standard factor-group analyses conflate.
