...
One practical implication comes from research on \enquote{ban-the-box} laws, which prohibit employers from asking about criminal history on initial applications. The research shows that people use the information they have: when one signal is removed, they may rely more heavily on others (including race). But the converse also holds: an informationally rich environment reduces the value of any single crude heuristic. If employers have work samples, structured interviews, and detailed portfolios, the marginal value of \enquote{sounds standard} as a sorting signal decreases. The intervention isn't just \enquote{stop discriminating on language}~-- it's \enquote{provide enough information that language becomes a less useful proxy}. This shifts the coordination structure itself.
