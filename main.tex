...
But O'Connor's logic implies that coordination will find \emph{something} to anchor on. You can't eliminate it; you can only redirect it. The normative question, then, is: redirect toward what? The framework suggests two criteria. First, toward properties that are \textbf{task-relevant}~-- conditioning on height for basketball or gregariousness for event organizing adds value; conditioning on accent for legal work (assuming the work isn't accent-dependent) doesn't. Second, toward properties that are \textbf{not ascribed}~-- conditioning on what people choose is less troubling than conditioning on what they're assigned. In S/A/I terms: shift conditioning away from \textsc{ascription} toward \textsc{situation} (what the task actually requires) and \textsc{identification} (what people have agency over). The intervention isn't eliminating coordination; it's designing environments where coordination tracks the right things.
