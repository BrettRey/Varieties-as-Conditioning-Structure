...
Two theoretical sources underpin this framework. From \textcite{wiese2023}: the primary unit of grammatical organisation isn't the named language but the \term{communicative situation}~-- the setting of communication as perceived by speakers. From \textcite{oconnor2019games}: the categories that stabilise in a population track \term{payoff space}, not \term{property space}. Varieties emerge as stable strategies in recurring coordination problems.
...
But Wiese's framework is primarily descriptive. It tells us that com-sits are the primary unit, but not \emph{why} certain partitions of com-sit space stabilise rather than others. For that, we need game theory.
...
The partitions that stabilise~-- what we call registers, dialects, and discourse communities~-- are stable strategies in these coordination games. They stabilise not because they mirror some intrinsic clustering of linguistic properties, but because they solve recurring social problems.

This explains why sociolinguistic categories are often messy. Variety boundaries are shaped by payoff relevance (identity, prestige, alignment) rather than by structural clustering alone. Multiple overlapping categories can coexist because different payoff spaces yield different stabilised partitions.

\subsection{Conventionality comes in degrees}

\textcite{oconnor2021-conventionality} develops this further by showing that conventions vary in their degree of arbitrariness. Some conventions are tightly constrained~-- only a few outcomes were ever plausible. Others are highly arbitrary~-- many alternatives could have stabilised.
...
In the game-theoretic framework: using these forms signalled membership and solved coordination problems (who belongs? who has authority to judge?). The forms stabilised because they served payoff-relevant functions for people who identified with that community.
...
The game-theoretic account explains \emph{why} certain partitions stabilise. The Bayesian account explains \emph{how} learners track them. The two are complementary: games explain population-level dynamics; Bayes explains individual learning.

The core idea is simple: learners keep track of which forms are appropriate in which circumstances, and they update their expectations as they accumulate experience. The formal machinery just makes this precise.
...
But we need to distinguish several things that \enquote{sounds wrong} might track. \term{Grammaticality} concerns morpho-syntactic form--value pairings: is this structure licensed in this variety? It has a fact of the matter~-- which is why you can have \emph{illusions} of ungrammaticality (center-embeddings that are well-formed but unparseable, garden paths that mislead). Without a fact, \enquote{illusion} makes no sense. \term{Appropriateness} is broader and slipperier: is this form fitting given the situation, the speaker's ascription, and the norms they're orienting to? It's evaluative and listener-model-relative~-- the S/A/I framework developed here. \term{Acceptability} is the phenomenology: what the listener actually experiences, informed by both grammaticality and appropriateness (and by processing factors that can fool the grammaticality judgment). And \term{correctness} is the prescriptive overlay: what gatekeepers enforce, what gets codified and moralised. These can diverge sharply~-- a form can be grammatical but inappropriate, appropriate but \enquote{incorrect} by some codified standard, or ungrammatical but accepted in a genre that licenses deviation.
...
This account of normativity aligns with what \textcite{pullum2019-normativity} calls the \term{constitutive} view of grammatical rules. Pullum argues that grammatical constraints aren't \term{regulative}~-- they don't tell you what you \emph{ought} to do in some moral sense. They're constitutive: they define what counts as participating in a particular practice. Speaking MTE involves patterning your utterances in the MTE way; that's not an obligation but a condition on the activity. Pullum's point is that grammaticality involves normativity without moral prescription. Departing from English grammar isn't like breaking a traffic law~-- it's more like drifting from what listeners expect, and that drift comes in degrees.

But the analogy Pullum draws with chess rules needs qualification. Chess rules are \emph{stipulated}: they were written down by someone with the authority to declare them. They're \emph{universal}: the same rules apply to everyone playing chess, anywhere. And they're \emph{explicit}: players can consult them. Grammatical constraints are none of these things.

Grammatical constraints are \term{emergent}, not stipulated. They arise from the coordination dynamics described in the preceding sections~-- stable distributions over forms that solve recurring communicative problems. No one declared that English would have subject-auxiliary inversion in questions; it emerged and stabilised. The constitutive force is real~-- the further you deviate, the less you're doing what others are doing~-- but the rules themselves weren't designed.

Grammatical constraints are \term{variety-indexed}, not universal. Which constraints apply depends on what you're conditioning on. The claim that double modals are ungrammatical is true for some English varieties and false for others. There's no variety-neutral grammaticality any more than there's a language-neutral one. Chess has one rule set; English is a family of overlapping conditioning structures, each with its own stable distributions.

Grammatical constraints are \term{tacit}, not explicit. Speakers follow them without being able to state them. You can reliably judge that \mention{*She is liking ice cream} sounds odd without knowing the first thing about stative verbs or the progressive. The constraint is internalised, not consulted.

So: Pullum is right that grammatical normativity is constitutive, not regulative. What you incur when you speak English is a commitment to pattern your speech in the English way, not a moral duty. But the constitutive rules themselves aren't stipulated, aren't universal, and aren't explicit. They're emergent conditioning structures~-- and which conditioning applies is exactly what the situation/ascription/identification framework specifies.
...
The minimal conditions for this are simple: (1) individuals recognise social categories, (2) they condition behaviour on category membership, and (3) they adapt to improve their own payoff. Once these conditions are met, coordination can stabilise on unequal outcomes~-- and it can do so even when most participants would prefer a fairer arrangement.
...
This analysis might sound like naturalising discrimination~-- reducing injustice to \enquote{just how coordination works}. It isn't. Equilibria aren't laws of nature; they're contingent on the structure of the game. Change the structure, and you change what's stable.
...
\begin{itemize}
    \item \textbf{Salience for mobilisation}: Making a category visible can be necessary for collective action, solidarity, and resistance. You can't fight discrimination against a group if the group isn't recognised.
...
This is a genuine tradeoff, not a mistake on either side. Communities aren't wrong to value salience; they're also not wrong to recognise what salience enables. The framework doesn't resolve the dilemma~-- it clarifies its structure.
...
This framework emerges from combining two insights. From \textcite{wiese2023}: the communicative situation is the primary unit of grammatical organisation, and variety concepts describe regularities across com-sits. From \textcite{oconnor2019games,oconnor2021-conventionality,oconnor2022contracts}: com-sits are coordination problems, the partitions that stabilise track payoff relevance, and category salience can stabilise inequity.
